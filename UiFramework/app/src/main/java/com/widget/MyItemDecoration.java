...
public class MyItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        paint.setColor(parent.getContext().getResources().getColor(R.color.black));
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if ((i + 1) % 4 == 1) {
                //根据这些点画条目的四周的线
                c.drawLine(x, y, x + width, y, paint);//上
                c.drawLine(x + width, y, x + width, y + height, paint);//右
                c.drawLine(x, y + height, x + width, y + height, paint);//下
            } else if ((i + 1) % 4 == 0) {
                //根据这些点画条目的四周的线
                c.drawLine(x, y, x + width, y, paint);//上
                c.drawLine(x, y, x, y + height, paint);//左
                c.drawLine(x, y + height, x + width, y + height, paint);//下
            } else {
                //根据这些点画条目的四周的线
                c.drawLine(x, y, x + width, y, paint);//上
